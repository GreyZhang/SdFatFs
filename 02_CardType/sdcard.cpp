...
static uint8_t sd_lld_sd_type_;
...
uint8_t sd_card_lld_init(void)
{
    ...
    // check SD version
    if ((sd_lld_cardCommand(CMD8, 0x1AA) & R1_ILLEGAL_COMMAND))
    {
        sd_lld_set_card_type(SD_CARD_TYPE_SD1);
    }
    else
    {
        ...
        {
            sd_lld_status_ = spi_just_spiRec();
        }
        if (sd_lld_status_ != 0XAA)
        {
            sd_lld_set_error(SD_CARD_ERROR_CMD8);
            goto fail;
        }
        sd_lld_set_card_type(SD_CARD_TYPE_SD2);
    }
    // initialize card and send host supports SDHC if SD2
    arg = (sd_lld_get_card_type() == SD_CARD_TYPE_SD2) ? 0X40000000 : 0;

    while ((sd_lld_status_ = sd_lld_cardAcmd(ACMD41, arg)) != R1_READY_STATE)
    {
        // check for timeout
        unsigned int d = millis() - t0;
        if (d > SD_INIT_TIMEOUT)
        {
            sd_lld_set_error(SD_CARD_ERROR_ACMD41);
            goto fail;
        }
    }
    // if SD2 read OCR register to check for SDHC card
    if (sd_lld_get_card_type() == SD_CARD_TYPE_SD2)
    {
        if (sd_lld_cardCommand(CMD58, 0))
        {
            sd_lld_set_error(SD_CARD_ERROR_CMD58);
            goto fail;
        }
        if ((spi_just_spiRec() & 0XC0) == 0XC0)
        {
            sd_lld_set_card_type(SD_CARD_TYPE_SDHC);
        }
